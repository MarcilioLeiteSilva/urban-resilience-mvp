...
export default function AdminDashboardPage() {
  ...
  if (loading) return <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 text-slate-400 text-sm font-bold animate-pulse">Carregando painel analítico administrativo...</div>;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900">
      {/* 1. Header Navigation Admin */}
      <header className="sticky top-0 z-50 w-full border-b bg-slate-900 text-white px-6 py-3 flex items-center justify-between shadow-sm">
           <div className="flex items-center gap-2">
                <span className="h-8 w-8 bg-white rounded-lg flex items-center justify-center text-slate-900 font-black text-xl">A</span>
                <div>
                     <h1 className="text-base font-black tracking-tight leading-4">Dashboard Admin</h1>
                     <p className="text-[10px] text-slate-400">Controle Operacional & Transparência</p>
                </div>
           </div>
           
           <nav className="flex items-center gap-4 text-sm font-semibold text-slate-300">
                <button className="text-white border-b-2 border-white pb-1">Painel</button>
                <button className="hover:text-white pb-1">Moderação</button>
                <button className="hover:text-white pb-1">Intervenções</button>
           </nav>
           ...
           <div className="grid grid-cols-12 gap-6 h-[500px]">
                ...
                <div className="col-span-12 lg:col-span-8 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden relative">
                     <div className="absolute inset-0 bg-slate-100/50 flex items-center justify-center text-slate-400 text-sm animate-pulse">
                          Carregando Mapa com Camadas Administrativas...
                     </div>
                </div>
